...
class Controller{
           ...
           public function __construct() {
               $this->setSmarty();
               if(MOUDULE==="Admin"){
                    $arr = array("LoginInfo","AdminLogin");
                    if(!in_array(ACTION, $arr)){ 
                         $admin_name = isset($_COOKIE["admin_name"])?$_COOKIE["admin_name"]:false;
                         if($admin_name ==false){
                        echo "<script>window.location.href=\"?a=Admin&c=Login&a=LoginInfo\";</script>";
